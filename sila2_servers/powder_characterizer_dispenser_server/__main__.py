...
_DEFAULT_IP = "0.0.0.0"  # Required for browser gateway access from another host.


def main(
    ip_address: str = Option(_DEFAULT_IP, "-a", "--ip-address", help="The IP address"),
    port: int = Option(50063, "-p", "--port", envvar="POWDER_DISPENSER_SILA_PORT", help="The port"),
    ...
    fake: bool = Option(False, "--fake", help="Use a no-hardware fake dispenser"),
):
    # validate parameters
    ...
    driver = None
    if fake:
        from .drivers.fake import FakePowderCharacterizerDispenser

        driver = FakePowderCharacterizerDispenser()
    server = Server(
